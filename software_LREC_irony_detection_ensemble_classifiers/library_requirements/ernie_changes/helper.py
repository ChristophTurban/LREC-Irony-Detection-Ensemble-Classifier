...
def get_features(tokenizer, sentences, labels):
    ...
    for i, sentence in enumerate(sentences):
        inputs = tokenizer.encode_plus(sentence, add_special_tokens=True, max_length=130)
        input_ids, token_type_ids = inputs['input_ids'], inputs['token_type_ids']
        padding_length = 130 - len(input_ids)
        ...
        assert 130 == len(attention_mask) == len(input_ids) == len(
            token_type_ids), f'{130}, {len(attention_mask)}, {len(input_ids)}, {len(token_type_ids)}'
